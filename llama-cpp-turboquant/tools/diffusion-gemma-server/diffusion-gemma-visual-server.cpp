...
int main(int argc, char ** argv) {
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        try {
            ...
            // fork-local (PR #24423 predates common_json): chat.h now takes common_json, not nlohmann
            const common_json creq = common_json::parse(raw);
            std::vector<common_chat_msg> messages = common_chat_msgs_parse_oaicompat(creq.at("messages"));
            common_chat_templates_inputs inputs;
            inputs.messages              = messages;
            if (creq.contains("tools")) {
                inputs.tools = common_chat_tools_parse_oaicompat(creq.at("tools"));
            }
