...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // TurboQuant weight types use their own fused paths; mmvf/mmf/mmvq/mmq have no TQ instances.
    if (src0->type == GGML_TYPE_TQ4_1S || src0->type == GGML_TYPE_TQ3_1S) {
        ...
        if (src0->type == GGML_TYPE_TQ4_1S) {
            ...
            // Gets tensor core throughput without the permanent 1.7x VRAM cost.
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_MUL_MAT_ID) {
            ...
            if (is_tq_w || ggml_cuda_mul_mat_id_needs_sync(node, cc)) {
                // TQ weights, and the mul_mat_id fallback path, synchronize the stream, so we cannot use CUDA graphs
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_SET_ROWS:
            {
                // TurboQuant types require head_dim divisible by the group size
