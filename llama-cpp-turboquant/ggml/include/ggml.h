...
extern "C" {
    ...
    enum ggml_type {
        ...
        GGML_TYPE_Q2_0    = 42,
        // ---------------------------------------------------------------
        // TurboQuant (localcode fork-local) types.
        //
        // These live in a reserved block at 64+ deliberately. They used to sit
        // at 41-45, which upstream then took for Q1_0/Q2_0 -- and because a
        // ggml type id is serialised into the GGUF tensor header, a collision
        // silently REINTERPRETS tensor data rather than failing to build. The
        // gap to 64 leaves upstream ~20 free ids of runway so the next upstream
        // bump cannot collide with us again.
        // ---------------------------------------------------------------
        GGML_TYPE_TURBO3_0 = 64, // TurboQuant 3-bit KV cache: 2-bit PolarQuant + 1-bit QJL
        GGML_TYPE_TURBO4_0 = 65, // TurboQuant 4-bit KV cache: 3-bit PolarQuant + 1-bit QJL
        GGML_TYPE_TURBO2_0 = 66, // TurboQuant 2-bit KV cache: 2-bit PolarQuant (no QJL)
        GGML_TYPE_TQ3_1S   = 67, // TurboQuant 3-bit weight: WHT-rotated 8-level Lloyd-Max, block_size=32
        GGML_TYPE_TQ4_1S   = 68, // TurboQuant 4-bit weight: WHT-rotated 16-level Lloyd-Max, block_size=32
        GGML_TYPE_COUNT    = 69,
    };
