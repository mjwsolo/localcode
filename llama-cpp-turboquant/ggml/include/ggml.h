...
extern "C" {
    ...
    enum ggml_type {
        ...
        GGML_TYPE_Q1_0    = 46, // upstream Q1_0 (relocated from 41 to avoid TurboQuant collision)
        GGML_TYPE_COUNT   = 47,
    };
    ...
    */
    ...
    // Fused MoE router: softmax + top-k + get_rows + weight normalization in one dispatch.
    // src[0] = logits [n_expert, n_tokens] (float32)
    // src[1] = weights_out [n_expert_used, n_tokens] (float32) - pre-allocated, kernel writes here
    // dst = selected_experts [n_expert_used, n_tokens] (int32)
    // n_expert_used is stored in op_params[0]
    GGML_API struct ggml_tensor * ggml_moe_router_fused(
            struct ggml_context * ctx,
            struct ggml_tensor  * logits,       // [n_expert, n_tokens]
            struct ggml_tensor  * weights_out,  // [n_expert_used, n_tokens] pre-allocated F32
            int                   n_expert_used);
