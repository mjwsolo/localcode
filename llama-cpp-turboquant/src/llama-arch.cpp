    ...
    switch (arch) {
        ...
        case LLM_ARCH_QWEN35:
            return {
                ...
                LLM_TENSOR_SSM_NORM,
                LLM_TENSOR_SSM_OUT,
                // Multi-Token-Prediction (nextn) tensors on the trailing MTP layer.
...
static const std::map<llm_tensor, llm_tensor_info> LLM_TENSOR_INFOS = {
    ...
    // NextN/MTP tensors are per-layer (they live on the trailing MTP block),
    // so they must be REPEATING — an OUTPUT classification forbids the layer
    // number their `blk.%d.nextn.*` names require. Loaded-but-unused: the
    // forward pass skips the MTP layer, so this is claim-only.
