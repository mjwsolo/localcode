...
llama_kv_cache::llama_kv_cache(
    ...
    for (uint32_t il = 0; il < n_layer; il++) {
        ...
        ggml_type layer_type_v = type_v;
        {
            ...
            // upstream turned n_layer from a field into an accessor (n_layer() vs
            // n_layer_all) during the 2026-04 -> 2026-08 window. TurboQuant's
            // layer-adaptive selection predates that.
    ...
    if (other) {
        ...
        // TurboQuant: default upstream's graph-level activation rotation OFF.
        // This fork rotates inside the KV kernels themselves (WHT via
        // simd_shuffle_xor in Metal/CUDA), so upstream's rotation would be a
        // second, redundant rotation, and its extra graph nodes have caused
        // hash-table overflow on some models. Set LLAMA_ATTN_ROT_DISABLE=0 to
        // opt back in to the upstream behaviour.
