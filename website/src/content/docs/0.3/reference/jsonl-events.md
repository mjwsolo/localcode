...
slug: 0.3/reference/jsonl-events
---
...
* `status` is `ok` when the turn's completion status is `completed`. Any other non-empty status (blocked, interrupted, stopped\_early, error, incomplete) is reported as `incomplete`.
