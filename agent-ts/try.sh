...
if [ "$ALL" = 1 ]; then
  ...
  LLAMA_BASE_URL="http://127.0.0.1:$PORT" \
    exec "$HERE/dist/localcode-agent" -a -e "$HERE/extensions/localcode.ts" -e "$HERE/extensions/localcode-brand.ts" -e "$HERE/extensions/localcode-safety.ts" -e "$HERE/extensions/localcode-web.ts" -e "$HERE/extensions/localcode-app.ts" \
...
cd "$PROJECT"
LLAMA_BASE_URL="http://127.0.0.1:$PORT" \
  "$HERE/dist/localcode-agent" -a -e "$HERE/extensions/localcode.ts" -e "$HERE/extensions/localcode-brand.ts" -e "$HERE/extensions/localcode-safety.ts" -e "$HERE/extensions/localcode-web.ts" -e "$HERE/extensions/localcode-app.ts" \
  --provider localcode --model "$MODEL" --models "localcode/*" --thinking off
