...
PORT=8082; LOG=$PWD/q-server.log; RESULTS=$PWD/results.tsv
...
one() { # $1=tool $2=task $3=idx
  ...
  if [ "$tool" = lc ]; then
    ...
    ( cd "$D" && timeout 900 "$PI" -a -e "$EXT" --provider localcode --model "$MODEL" \
        --thinking off --no-session --append-system-prompt "$APPEND" -p "$goal" >/dev/null 2>&1 )
  fi
...
for task in retry parser; do
  for i in $(seq 1 $N); do one lc "$task" "$i"; one pi "$task" "$i"; one pi2 "$task" "$i"; done
done
echo DONE
