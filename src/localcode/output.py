...
class OutputManager:
    ...
    def update_turn_tokens(
        ...
        """Forward llama-server's `usage` info (prompt + completion token
        counts) to the UI so the per-turn summary can show real
        in/out counts instead of estimates.

        Called once per round from `agent/loop.py` after `stream_done`.
        Multi-tool turns fire it N times; the chat screen accumulates
        and resets at turn boundaries.
        """
        self._emit_event(
            "turn_tokens",
            prompt_tokens=str(int(prompt_tokens or 0)),
            completion_tokens=str(int(completion_tokens or 0)),
            total_tokens=str(int(total_tokens or 0)),
        )

    def tool_preview(self, name: str, args_chars: int = 0, args_snippet: str = "") -> None:
        """Mid-stream signal that a tool call is forming.

        Fires the moment the model commits to a tool name and again as the
        args grow. Lets the TUI render a `▪ Write` placeholder + size
        progress IMMEDIATELY instead of waiting for end-of-stream — without
        this, a 5K-line file write looks like a 3-minute hang.

        `args_snippet` carries the first ~256 chars of the accumulating
        JSON args so the UI can regex-extract `"path": "x"` and show
        `writing(pyproject.toml… 3 KB)` instead of the generic
        `writing(streaming… 3 KB)`.

        Distinct from `log_tool`: no state mutation, no stdout write, no
        `tool_actions.append`. Just an event for the UI to update its
        floating active-step widget.
        """
        self._emit_event(
            "tool_preview",
            name=name,
            chars=str(args_chars),
            snippet=args_snippet[:256],
        )

    def log_tool(self, name: str, args: str = "") -> int:
        """Log a tool call with agent-style formatting."""
        self._stop_indicator()
    ...
    def _filter_noise(text: str) -> str:
        ...
        filtered = [
            ...
            and "MallocStackLoggingNoCompact" not in l
            and "MallocNanoZone" not in l
            and "can't turn off malloc stack logging" not in l
        ]
        return "\n".join(filtered)
    ...
    def stream(self, chunk: str) -> None:
        ...
        TUI mode (event_callback is set): emits a `content` event for
        the chat-log widget to consume — DOES NOT write to stdout.
        Earlier code wrote to stdout unconditionally, which (a) doubled
        the model's prose into both the TUI render path AND raw stdout,
        and (b) leaked the raw bytes to the visible terminal whenever
        Textual's alt-screen was in a transitional state (startup race,
        mid-resize, paste handoff). Real failure 2026-04-26: user saw
        ~14 sentences of model narration printed raw to the terminal
        outside the chat log because the TUI hadn't fully attached yet.

        CLI mode (no event callback): same as before — soft-wrap and
        write to stdout for the no-Textual fallback.
        """
        ...
        # In TUI mode the chat-log widget renders streamed content
        # exclusively. Skipping stdout here is the difference between
        # "model prose appears in the chat log only" and "model prose
        # leaks to the raw terminal whenever the alt-screen flickers".
        if self._event_callback is not None:
            return
        cols = _cols()
    ...
    def _run_indicator(self) -> None:
        # CLI-mode spinner removed — TUI owns the terminal and renders
        # its own thinking indicator. start/stop scaffolding kept for
        # legacy call sites; this thread now just idles until stopped.
