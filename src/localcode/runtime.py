...
class LocalCodeRuntimeGateway(_DiffusionMixin):
    ...
    def llama_server_command(self, model_path: str, port: int = 8081) -> list[str]:
        ...
        # N-gram/lookup decoding causes infinite verbatim repetition: once
        # the model emits a phrase, the n-gram drafter re-proposes it and
        # greedy verification accepts the whole block, so the loop
        # accelerates instead of breaking. Opt into a real draft model
        # (lossless) or an explicit spec_type instead.
        ...
        for attempt in range(_attempt_budget):
            try:
                ...
                if (
                    ...
                    # re-sending a byte-identical request (which would just
                    # re-sample the same degenerate path — note some models'
                    # base temperature is already 1.0, so a clamped bump
                    # alone is a no-op). Three independent levers:
