...
class HistoryDB:
    ...
    def conn(self) -> sqlite3.Connection:
        if self._conn is None:
            ...
            # across them, so interleaved execute/commit can tear transactions.
    ...
    def _ensure_schema(self) -> None:
        ...
        if current < DB_VERSION:
            ...
            self.conn.commit()
        # Additive migrations for older local DBs.
        cols = {
            row["name"]
            for row in self.conn.execute("PRAGMA table_info(history)").fetchall()
        }
        for name, ddl in (
            ("task_id", "ALTER TABLE history ADD COLUMN task_id TEXT DEFAULT ''"),
            ("task_status", "ALTER TABLE history ADD COLUMN task_status TEXT DEFAULT ''"),
            ("task_stage", "ALTER TABLE history ADD COLUMN task_stage TEXT DEFAULT ''"),
            ("blocked_reason", "ALTER TABLE history ADD COLUMN blocked_reason TEXT DEFAULT ''"),
            ("goal_type", "ALTER TABLE history ADD COLUMN goal_type TEXT DEFAULT ''"),
            ("goal_summary", "ALTER TABLE history ADD COLUMN goal_summary TEXT DEFAULT ''"),
            ("completion_status", "ALTER TABLE history ADD COLUMN completion_status TEXT DEFAULT ''"),
            ("task_kind", "ALTER TABLE history ADD COLUMN task_kind TEXT DEFAULT ''"),
            ("task_slug", "ALTER TABLE history ADD COLUMN task_slug TEXT DEFAULT ''"),
        ):
            if name not in cols:
    ...
    def _record_turn_locked(self, entry: HistoryEntry) -> None:
        self.conn.execute("""
            INSERT INTO history (
                ...
                files_changed, diff_summary, task_id, task_status, task_stage, blocked_reason,
                goal_type, goal_summary, completion_status, task_kind, task_slug
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
    ...
    def record_user_prompt(self, session_id: str, repo_root: str,
                           prompt: str, intent: str = "", model: str = "",
                           goal_type: str = "", goal_summary: str = "",
    ...
    def record_assistant_response(self, session_id: str, repo_root: str,
                                  ...
                                  model: str = "", files_changed: list[str] | None = None,
                                  task_id: str = "", task_status: str = "",
                                  task_stage: str = "",
                                  blocked_reason: str = "",
                                  goal_type: str = "", goal_summary: str = "",
