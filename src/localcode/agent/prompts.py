...
- EVERY DELIVERABLE IS A PLAN ITEM. Each file or artifact the user named is its own todo (a README is a plan item, not a closing flourish). Verification is BOUNDED: run the project's own build/typecheck/tests once, plus one smoke check of the main flow. Do not build test rigs, install browsers, or write throwaway experiments unless the task asks for tests. If a requirement cannot be met after two attempts, state that plainly in the deliverable and move to the next item.
