...
CHOICES: list[ModelChoice] = [
    ...
    ModelChoice(
        ...
        size_gb=17.9,
        active_params="27B (dense)",
        architecture="qwen35",
        license="Apache 2.0",
        humaneval_pass_at_1=None,
        notes="Qwen 3.8 27B — a DENSE vision-language model (not MoE): understands images, thinking on by default (per-request toggle), native 262K context. Because it's dense, decode speed scales with ALL 27B params, so it's heavier than the 3B-active MoE entries — recommend >=32 GB unified memory (comfortable on 48 GB+). UD-Q4_K_XL keeps attention/embedding layers above 4-bit for quality. Pair with the F16 mmproj for image input. qwen35 arch is natively supported by the TurboQuant server (turbo4 KV + context checkpoints).",
    ...
    ModelChoice(
        ...
        license="Apache 2.0",
        humaneval_pass_at_1=None,
        notes="High-RAM workstation pick (>=64 GB unified memory; sweet spot on 128 GB Apple Silicon). Near-lossless UD-Q8_K_XL of the dense Qwen 3.8 27B vision-language model (~99% of BF16 quality). Thinking on by default, native 262K context, image input via the F16 mmproj. Dense 27B -> heavier decode than the MoE entries; the extra RAM buys quality, not speed.",
        mmproj_filename="mmproj-Qwen3.8-27B-F16.gguf",
        mmproj_size_gb=0.9,
        mmproj_hf_filename="mmproj-F16.gguf",
    ),
