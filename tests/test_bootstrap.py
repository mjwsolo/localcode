"""Tests for gem.bootstrap — TurboQuant source detection, binary path, install plans."""
...
class TestFindTurboquantSource:
    ...
    def test_finds_source_in_repo_root(self, tmp_path: Path) -> None:
        """If llama-cpp-turboquant/ is beside the gem package, it should be found."""
        ...
        # Patch __file__ so the function thinks it's inside fake_repo/src/gem/bootstrap.py
        fake_file = fake_repo / "src" / "gem" / "bootstrap.py"
        fake_file.parent.mkdir(parents=True)
        fake_file.touch()
        with patch("gem.bootstrap.__file__", str(fake_file)):
            result = _find_turboquant_source()
        assert result == tq

    def test_returns_none_when_not_present(self, tmp_path: Path) -> None:
        fake_file = tmp_path / "src" / "gem" / "bootstrap.py"
        fake_file.parent.mkdir(parents=True)
        fake_file.touch()
        with patch("gem.bootstrap.__file__", str(fake_file)):
    ...
    def test_finds_source_in_home_dir(self, tmp_path: Path) -> None:
        ...
        (tq / "CMakeLists.txt").write_text("cmake_minimum_required(VERSION 3.10)")
        fake_file = tmp_path / "elsewhere" / "src" / "gem" / "bootstrap.py"
        fake_file.parent.mkdir(parents=True)
        fake_file.touch()
        with patch("gem.bootstrap.__file__", str(fake_file)):
...
class TestBuildTurboquant:
    ...
    def test_fails_without_cmake(self, tmp_path: Path) -> None:
        ...
        (tq / "CMakeLists.txt").write_text("cmake_minimum_required(VERSION 3.10)")
        with patch("gem.bootstrap._find_turboquant_source", return_value=tq):
            with patch("gem.bootstrap._ensure_cmake", return_value=False):
                ok, msg = build_turboquant()
        assert ok is False
