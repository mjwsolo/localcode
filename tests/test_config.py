"""Tests for gem.config — loading, saving, env overrides, directory creation."""
...
class TestEnsureHomeDirs:
    ...
    def test_creates_expected_subdirs(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_test_home")
        try:
    ...
    def test_idempotent(self, tmp_path: Path) -> None:
        ...
        os.environ["GEM_HOME"] = str(tmp_path / "gem_test_home2")
        try:
            ensure_home_dirs()
...
class TestGetHomeDir:
    """Verify GEM_HOME env override and default behaviour."""

    def test_override_via_env(self, tmp_path: Path) -> None:
        custom = tmp_path / "custom_gem"
        os.environ["GEM_HOME"] = str(custom)
        try:
    ...
    def test_default_is_dot_gem(self, monkeypatch) -> None:
        ...
        assert get_home_dir() == Path.home() / ".gem"
...
class TestInitConfigFile:
    ...
    def test_creates_default_config(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_init")
        try:
    ...
    def test_does_not_overwrite_existing(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_init2")
        try:
            path = init_config_file()
...
class TestLoadConfig:
    ...
    def test_returns_appconfig(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_load")
        try:
            cfg = load_config()
    ...
    def test_kv_cache_defaults(self, tmp_path: Path) -> None:
        ...
        os.environ["GEM_HOME"] = str(tmp_path / "gem_load3")
        try:
            cfg = load_config()
...
class TestSaveConfig:
    ...
    def test_roundtrip(self, tmp_path: Path) -> None:
        ...
        os.environ["GEM_HOME"] = str(tmp_path / "gem_rt")
        try:
            original = AppConfig(
                ...
                voice=VoiceConfig(tts_provider="piper"),
            ...
            assert reloaded.browser.enabled is False
...
class TestEnvOverrides:
    """Verify that GEM_* environment variables override config file values."""

    def test_gem_provider_env(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_env")
        os.environ["GEM_PROVIDER"] = "llama_cpp"
        try:
            cfg = load_config()
            assert cfg.runtime.provider == "llama_cpp"
        finally:
            os.environ.pop("GEM_HOME", None)
            os.environ.pop("GEM_PROVIDER", None)

    def test_gem_model_env(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_env2")
        os.environ["GEM_MODEL"] = "custom-gemma"
        try:
            cfg = load_config()
            assert cfg.runtime.model == "custom-gemma"
        finally:
            os.environ.pop("GEM_HOME", None)
            os.environ.pop("GEM_MODEL", None)

    def test_gem_llama_cpp_binary_env(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_env3")
        os.environ["GEM_LLAMA_CPP_BINARY"] = "/opt/llama/llama-server"
        try:
            cfg = load_config()
            assert cfg.runtime.llama_cpp_binary == "/opt/llama/llama-server"
        finally:
            os.environ.pop("GEM_HOME", None)
            os.environ.pop("GEM_LLAMA_CPP_BINARY", None)

    def test_gem_temperature_env(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_env4")
        os.environ["GEM_TEMPERATURE"] = "0.99"
        try:
            cfg = load_config()
            assert cfg.runtime.temperature == pytest.approx(0.99)
        finally:
            os.environ.pop("GEM_HOME", None)
            os.environ.pop("GEM_TEMPERATURE", None)

    def test_gem_base_url_env(self, tmp_path: Path) -> None:
        os.environ["GEM_HOME"] = str(tmp_path / "gem_env5")
        os.environ["GEM_BASE_URL"] = "http://192.168.1.100:8080"
        try:
            cfg = load_config()
            assert cfg.runtime.base_url == "http://192.168.1.100:8080"
        finally:
            os.environ.pop("GEM_HOME", None)
            os.environ.pop("GEM_BASE_URL", None)
