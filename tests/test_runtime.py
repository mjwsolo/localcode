...
class TestCollapseRetry:
    ...
    def test_retry_escalates_sampler_not_a_noop(self) -> None:
        ...
        the initial attempt. A temperature nudge clamped to 1.0 is a no-op
        for models whose base temperature is already 1.0 — a byte-identical
        retry just reproduces the same collapse. The retry must escalate:
        a fresh seed + a higher repeat_penalty than the base payload
        carries."""
